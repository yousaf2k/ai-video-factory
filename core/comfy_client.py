...
def get_output_file_path(filename, project_id=None, subfolder=None, file_type='video'):
    """
    Resolve the local absolute path for a file generated by ComfyUI.
    
    Args:
        filename: The name of the file or a dictionary containing 'filename' and 'subfolder'
        project_id: Optional project ID to help resolve relative paths
        subfolder: Optional subfolder within ComfyUI output directory
        file_type: 'video', 'image', or 'temp'
        
    Returns:
        str: Absolute path to the file
    """
    # Handle dictionary input (common mistake in caller)
    if isinstance(filename, dict):
        logger.debug(f"get_output_file_path received a dict instead of string: {filename}")
        if not subfolder:
            subfolder = filename.get('subfolder')
        filename = filename.get('filename')

    # Guard against coroutines (root cause of 'stat: path should be string... not coroutine' error)
    if hasattr(filename, '__await__'):
        import traceback
        logger.error(f"CRITICAL BUG: get_output_file_path received a coroutine for filename!")
        logger.error(f"Coroutine: {filename}")
        logger.error(f"Traceback:\n{''.join(traceback.format_stack())}")
        raise TypeError(f"get_output_file_path: filename should be string, not coroutine: {filename}")
