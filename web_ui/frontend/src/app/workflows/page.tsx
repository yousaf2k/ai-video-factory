...
import { useWorkflows, useWorkflowContent, useUpdateWorkflowContent } from '@/hooks/useWorkflows';
import { Save, RefreshCw, FileJson, ChevronRight, Settings } from 'lucide-react';
import { cn } from '@/lib/utils';

export default function WorkflowsPage() {
  const { data: workflows, isLoading: isLoadingWorkflows } = useWorkflows();
  const [selectedWorkflow, setSelectedWorkflow] = useState<{ category: string, filename: string } | null>(null);
  
  const { data: content, isLoading: isLoadingContent, error: loadError } = useWorkflowContent(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`Failed to update workflow: ${error.response?.data?.detail || error.message}`);
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          View and edit the JSON workflow templates for image, video, and voice generation
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 border rounded-lg overflow-hidden flex flex-col h-[calc(100vh-250px)]">
          <div className="bg-muted p-3 font-semibold border-b">Workflow Categories</div>
              ...
              <div key={category} className="mb-2">
                ...
                <div className="space-y-1 p-1">
                  {workflows?.[category]?.map((workflow) => (
                    <button
                      key={workflow.id}
                      onClick={() => setSelectedWorkflow({ category, filename: workflow.filename })}
                      className={cn(
                        "w-full text-left px-3 py-2 rounded-md text-sm flex items-center justify-between transition-colors",
                        selectedWorkflow?.filename === workflow.filename && selectedWorkflow?.category === category
          ...
          {selectedWorkflow ? (
            <>
              <div className="bg-muted p-3 flex items-center justify-between border-b">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={handleSave}
                    disabled={updateWorkflowMutation.isPending || isLoadingContent}
              ...
              <div className="flex-1 relative">
                {isLoadingContent ? (
                  ...
                  <textarea
                    ...
                    className={cn(
                      "w-full h-full p-6 font-mono text-sm resize-none focus:outline-none bg-card",
            ...
            <div className="flex-1 flex flex-center flex-col items-center justify-center p-12 text-center text-muted-foreground">
              ...
              <p className="max-w-xs">
                Choose a workflow from the sidebar to view or edit its JSON configuration.
              </p>
            </div>
          )}
        </div>
